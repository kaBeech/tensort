...
import Data.Tensort.Utils.Types (Memory (..), TensorStack, TensortProps (..), WonkyState)
...
reduceTensorStacks :: [TensorStack] -> TensortProps -> WonkyState -> (TensorStack, WonkyState)
reduceTensorStacks tensorStacks tsProps wonkySt = do
  let (newTensorStacks, wonkySt') = reduceTensorStacksSinglePass tensorStacks tsProps wonkySt
  if length newTensorStacks <= bytesize tsProps
    then createTensor (TensorMem newTensorStacks) (subAlgorithm tsProps) wonkySt'
    else reduceTensorStacks newTensorStacks tsProps wonkySt'
...
reduceTensorStacksSinglePass :: [TensorStack] -> TensortProps -> WonkyState -> ([TensorStack], WonkyState)
reduceTensorStacksSinglePass tensorStacks tsProps wonkySt = foldr acc ([], wonkySt) (splitEvery (bytesize tsProps) tensorStacks)
  where
    acc :: [TensorStack] -> ([TensorStack], WonkyState) -> ([TensorStack], WonkyState)
    acc tensorStack (newTensorStacks, wonkySt') = do
      let (tensor, wonkySt'') = createTensor (TensorMem tensorStack) (subAlgorithm tsProps) wonkySt'
      (newTensorStacks ++ [tensor], wonkySt'')
